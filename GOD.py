    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"Please specify a valid number and unit of time (hours/days) script by OWNER- @GODNEXTYT..."
        else:
            response = "Usage: /genkey <amount> <hours/days>"
    else:
        response = f"ONLY OWNER CAN USE💀OWNER OWNER- @GODNEXTYT..."
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = f"✅Key redeemed successfully! Access granted until: {users[user_id]} OWNER- @GODNEXTYT..."
        else:
            response = f"Invalid or expired key buy from OWNER- @GODNEXTYT..."
    else:
        response = f"Usage: /redeem <key> if you don't  have  buy from  @GODNEXTYT..."
...
async def allusers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        if users:
            ...
            response = f"No data found OWNER- @GODNEXTYT ..."
    else:
        response = f"ONLY OWNER CAN USE.OWNER- @GODNEXTYT..."
    await update.message.reply_text(response)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    response = (
        "Welcome to the Flooding Bot by OWNER- @GODNEXTYT...! Here are the available commands:\n\n"
        "Admin Commands:\n"
